...
fn notify(title: &str, message: &str, urgency: &str) -> Result<(), std::io::Error> {
...
fn main() -> battery::Result<()> {
    ...
    if prev_plugged {
        if prev_percent == 100 {
            let _ = notify("Battery", "Plugged In: battery full", "normal");
        } else {
            let _ = notify("Battery Charging", &message, "normal");
        }
    ...
    loop {
        ...
        let plugged = battery.state() != State::Discharging;
        let current_percent = battery.state_of_charge().get::<percent>().floor() as u8;
        let message = format!("Current level: {}%", current_percent);
        ...
        if plugged != prev_plugged {
            if plugged {
                if current_percent == 100 {
                    ...
                    let _ = notify("Battery Plugged In", 
                                   &format!("Charging: {}%", current_percent), "normal");
                }
            } else {
                if current_percent <= CRITICAL_LEVEL {
                    let _ = notify("Battery Unplugged - CRITICAL", &message, "critical");
                } else if current_percent <= WARNING_LEVEL {
                    let _ = notify("Battery Unplugged - WARNING", &message, "normal");
                } else {
                    let _ = notify("Battery Unplugged", &message, "normal");
                }
            }
